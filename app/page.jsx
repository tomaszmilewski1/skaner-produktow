...
export default function Home() {
  ...
  const loadProductData = async (code) => {
    ...
    const updated = [{ ean: code, name: pName || "Nieznany produkt" }, ...recentScans.filter((i) => i.ean !== code)].slice(0, 5);
  ...
  const likeReview = async (id, cur) => {
    await supabase.from("reviews").update({ helpful_count: (cur || 0) + 1 }).eq("id", id);
    fetchReviews(ean);
  };
  ...
  return (
    <main className="min-h-screen p-3 flex flex-col items-center bg-slate-950 text-slate-100 font-sans">
      {/* GÓRNY PASEK Z PRZYCISKIEM NOWEGO SKANU */}
      <header className="w-full max-w-md flex justify-between items-center mb-3 pb-2 border-b border-slate-800 text-xs">
        <div className="flex gap-1.5 items-center">
          ...
          <button onClick={() => { setActiveTab("list"); if (user) supabase.from("shopping_list").select("*").eq("user_id", user.id).then(({ data }) => setShoppingList(data || [])); }} className={`px-2.5 py-1.5 rounded-lg font-bold ${activeTab === "list" ? "bg-blue-600 text-white" : "bg-slate-900 text-slate-400"}`}>
            🛒 ({shoppingList.length})
          </button>
      ...
      {activeTab === "profile" && (
        <div className="w-full max-w-md bg-slate-900 p-4 rounded-xl border border-slate-800 text-xs">
          {!user ? (
            ...
            <div>
              <p className="font-bold text-white mb-2">Zalogowano jako: {user.email}</p>
            </div>
          )}
        </div>
      )}

      {/* WIDOK: SKANER I KARTA PRODUKTU */}
      {activeTab === "scan" && (
        <>
          {!ean && (
            <div className="w-full max-w-md flex flex-col gap-3">
              ...
              <div className="bg-slate-900 p-2.5 rounded-xl border border-slate-800 flex gap-2">
                <input type="text" placeholder="Wpisz kod EAN ręcznie..." value={manualQuery} onChange={(e) => setManualQuery(e.target.value)} className="bg-slate-800 border border-slate-700 p-1.5 rounded text-xs flex-1 text-white outline-none" />
                <button onClick={() => manualQuery && loadProductData(manualQuery.trim())} className="bg-blue-600 px-3 py-1.5 rounded text-xs font-bold text-white">Szukaj</button>
              </div>

              {recentScans.length > 0 && (
                <div className="bg-slate-900 p-2.5 rounded-xl border border-slate-800 text-xs">
                  <span className="text-[10px] text-slate-500 uppercase font-bold block mb-1">Ostatnie skany:</span>
                  {recentScans.map((s) => (
                    <button key={s.ean} onClick={() => loadProductData(s.ean)} className="block text-blue-400 hover:underline truncate w-full text-left py-0.5">
                      {s.name} ({s.ean})
                    </button>
                  ))}
                </div>
          ...
          {product && (
            <div className="w-full max-w-md flex flex-col gap-3">
              <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 flex flex-col items-center text-xs">
                {product.img && <img src={product.img} alt="" className="w-24 h-24 object-contain mb-2 bg-white rounded p-1" />}
                <h2 className="text-base font-bold text-center text-white mb-1">{product.name}</h2>
                <p className="text-slate-500 text-[10px] mb-2">EAN: {ean}</p>
                ...
                {/* EDYCJA NAZWY TOWARU */}
                <input type="text" placeholder="Zmień nazwę produktu..." value={customName} onChange={(e) => setCustomName(e.target.value)} className="w-full bg-slate-800 border border-slate-700 p-1.5 rounded text-xs text-white mb-3 outline-none" />
                ...
                {reviews.map((r) => (
                  <div key={r.id} className="border-b border-slate-800 py-1.5 last:border-0">
                    <div className="flex justify-between text-amber-400 font-bold">
                      ...
                      <button onClick={() => likeReview(r.id, r.helpful_count)} className="text-[10px] text-slate-400 hover:text-white">
                        👍 Pomocna ({r.helpful_count || 0})
                      </button>
                    </div>
